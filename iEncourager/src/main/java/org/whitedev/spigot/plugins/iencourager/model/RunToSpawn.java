...
public class RunToSpawn {
	...
	//<editor-fold defaultstate="collapsed" desc="Test calculateNextSleep() Method">
	...
	/*private Long calculateNextSleep() {
	    ...
	    GregorianCalendar gregorian2 = (GregorianCalendar) Calendar.getInstance();
	    gregorian2.set(Calendar.getInstance().get(Calendar.YEAR),
		    Calendar.getInstance().get(Calendar.MONTH),
		    Calendar.getInstance().get(Calendar.DAY_OF_MONTH),
		    23, 43, 0);// a las 8:05
	    Long gregorian2TimeInMillis = gregorian2.getTimeInMillis();

	    Long millisecondsAfterNow = gregorianTimeInMillis - Calendar.getInstance().getTimeInMillis() < 0 ? gregorian2TimeInMillis - Calendar.getInstance().getTimeInMillis() : gregorianTimeInMillis - Calendar.getInstance().getTimeInMillis();

	    Calendar c = Calendar.getInstance();
	    c.setTimeInMillis(millisecondsAfterNow);
	    plugin.getLogger().info("Time until next event:" + c.get(Calendar.MINUTE) + "mins " + c.get(Calendar.SECOND) + "secs");

	    return millisecondsAfterNow;
	}*/
//</editor-fold>
	...
	public void run() {
		...
		Bukkit.getServer().broadcastMessage(IEncouragerConfigFile.getConfigValue(
	...
	public void run() {
		...
		Bukkit.getServer().broadcastMessage(ChatColor.RED + IEncouragerConfigFile.
    ...
    public static RunToSpawn getSingleton() {
	if (singleton == null) {
    ...
    public static RunToSpawn getSingleton(JavaPlugin plugin) {
	if (singleton == null) {
	    singleton = new RunToSpawn();
	}
